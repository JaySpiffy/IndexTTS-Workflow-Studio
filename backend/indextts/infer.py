...
os.environ.setdefault('HF_HUB_CACHE', './shared/models/checkpoints/hf_cache')
import time
...
class IndexTTS:
    def __init__(
        ...
        if self.use_cuda_kernel:
            ...
            try:
                ...
                self.use_cuda_kernel = False
        # PATCHED: Handle both old and new config formats
        if hasattr(self.cfg, 'bigvgan'):
            bigvgan_config = self.cfg.bigvgan
            bigvgan_checkpoint = self.cfg.bigvgan_checkpoint
        elif hasattr(self.cfg, 'vocoder'):
            bigvgan_config = self.cfg.vocoder
            bigvgan_checkpoint = self.cfg.bigvgan_checkpoint if hasattr(self.cfg, 'bigvgan_checkpoint') else 'bigvgan.pth'
        else:
            raise KeyError("Neither 'bigvgan' nor 'vocoder' configuration found in config")
        
        self.bigvgan = Generator(bigvgan_config, use_cuda_kernel=self.use_cuda_kernel)
        
        # Try to load from local file first, then fall back to Hugging Face Hub
        self.bigvgan_path = os.path.join(self.model_dir, bigvgan_checkpoint)
        if os.path.exists(self.bigvgan_path):
            print(f">> Loading BigVGAN from local file: {self.bigvgan_path}")
            vocoder_dict = torch.load(self.bigvgan_path, map_location="cpu")
            try:
                self.bigvgan.load_state_dict(vocoder_dict["generator"], strict=True)
            except RuntimeError as e:
                print(f">> [WARNING] Strict loading failed: {e}")
                print(">> [INFO] Attempting non-strict loading...")
                try:
                    # Try non-strict loading
                    missing_keys, unexpected_keys = self.bigvgan.load_state_dict(vocoder_dict["generator"], strict=False)
                    if missing_keys:
                        print(f">> [INFO] Missing keys ({len(missing_keys)}): {missing_keys[:5]}...")
                    if unexpected_keys:
                        print(f">> [INFO] Unexpected keys ({len(unexpected_keys)}): {unexpected_keys[:5]}...")
                except Exception as e2:
                    print(f">> [ERROR] Non-strict loading also failed: {e2}")
                    raise
        else:
            print(f">> BigVGAN file not found locally, loading from Hugging Face Hub: {bigvgan_config.name}")
            try:
                # Load from Hugging Face Hub
                from indextts.BigVGAN.bigvgan import BigVGAN as BigVGANLoader
                from indextts.BigVGAN.env import load_hparams_from_json
                import tempfile
                import json
                ...
                # Load the Hugging Face config
                hf_config = load_hparams_from_json(config_file)
                
                # Merge our local config with the Hugging Face config
                # Local config takes precedence
                merged_config = dict(hf_config)
                for key, value in bigvgan_config.items():
                    if key not in ['name', 'type']:
                        merged_config[key] = value
                
                # Convert to AttrDict
                from indextts.BigVGAN.env import AttrDict
                merged_config = AttrDict(merged_config)
                
                # Create the model with merged config
                self.bigvgan = BigVGANLoader(merged_config, use_cuda_kernel=self.use_cuda_kernel)
                ...
                try:
                    self.bigvgan.load_state_dict(checkpoint_dict["generator"], strict=True)
                except RuntimeError as e:
                    print(f">> [WARNING] Strict loading failed: {e}")
                    print(">> [INFO] Attempting non-strict loading...")
                    try:
                        # Try non-strict loading
                        missing_keys, unexpected_keys = self.bigvgan.load_state_dict(checkpoint_dict["generator"], strict=False)
                        if missing_keys:
                            print(f">> [INFO] Missing keys ({len(missing_keys)}): {missing_keys[:5]}...")
                        if unexpected_keys:
                            print(f">> [INFO] Unexpected keys ({len(unexpected_keys)}): {unexpected_keys[:5]}...")
                        ...
                        print(f">> [ERROR] Non-strict loading also failed: {e2}")
                        print(">> [INFO] Attempting to remove weight norm and reload...")
                        try:
                            self.bigvgan.remove_weight_norm()
                            self.bigvgan.load_state_dict(checkpoint_dict["generator"], strict=False)
                            print(">> [INFO] Successfully loaded after removing weight norm")
                        except Exception as e3:
                            print(f">> [ERROR] All loading attempts failed: {e3}")
                            raise
                
                print(f">> BigVGAN loaded successfully from Hugging Face Hub")
            except Exception as e:
                print(f">> Failed to load BigVGAN from Hugging Face Hub: {e}")
                print(f">> Please download the BigVGAN model and place it at: {self.bigvgan_path}")
                raise FileNotFoundError(f"BigVGAN model not found at {self.bigvgan_path} and failed to load from Hugging Face Hub")
        self.bigvgan = self.bigvgan.to(self.device)
        # remove weight norm on eval mode (only if not already removed)
        try:
            self.bigvgan.remove_weight_norm()
            print(">> Weight norm removed from BigVGAN")
        except ValueError:
            print(">> [INFO] Weight norm already removed from BigVGAN")
        self.bigvgan.eval()
    ...
    def _set_progress(self, value, desc):
        if self.progress_callback is not None:
            self.progress_callback(value, desc=desc)
    ...
    def infer_fast(self, audio_prompt, text, output_path, verbose=False, max_text_tokens_per_segment=100,
        ...
        self._set_progress(0, "starting fast inference...")
        if verbose:
        ...
        self._set_progress(0.1, "text processing...")
        ...
        for item_tokens in all_text_tokens:
            ...
            # gpt speech
            self._set_progress(0.2 + 0.3 * processed_num / all_batch_num,
        ...
        # gpt latent
        self._set_progress(0.5, "gpt latents inference...")
        all_idxs = []
        all_latents = []
        has_warned = False
        ...
        self._set_progress(0.7, "bigvgan decoding...")
        ...
        # wav audio output
        self._set_progress(0.9, "saving audio...")
        ...
        if output_path:
            ...
            # Return audio data as tuple (sample_rate, numpy_array)
    ...
    def infer(self, audio_prompt, text, output_path, verbose=False, max_text_tokens_per_segment=120,
        ...
        self._set_progress(0, "starting inference...")
        if verbose:
            print(f"origin text:{text}")
        start_time = time.perf_counter()
        ...
        for sent in segments:
            ...
            # print(text_len)
            progress += 1
            self._set_progress(0.2 + 0.4 * (progress - 1) / len(segments),
            ...
            with torch.no_grad():
                ...
                if verbose:
                    ...
                    print(f"code len: {code_lens}")
                self._set_progress(0.2 + 0.4 * progress / len(segments),
