...
class IndexTTS2:
    def __init__(
        ...
        try:
            ...
            if self.use_deepspeed:
                print(">> DeepSpeed inference acceleration enabled")
        except Exception as e:
            if use_deepspeed:
                print(f">> DeepSpeed init failed. Falling back to normal inference. Error: {e}")
    ...
    def _get_qwen_emo(self):
        if self.qwen_emo is None:
            print(">> Loading Qwen emotion model on first use...")
            self.qwen_emo = QwenEmotion(self.qwen_emo_path)
        return self.qwen_emo

    def release_unused_memory(self, clear_prompt_cache=False):
        """
        Release per-request tensors and ask the backend allocator to return
        currently-unused GPU cache to the system.
        """
        try:
            ...
            elif str(self.device) == "mps" and torch.backends.mps.is_available():
                torch.mps.empty_cache()
        except Exception as e:
            print(f">> Warning: failed to release unused memory cleanly: {e}")

    # 原始推理模式
    def _run_with_deterministic_seed(self, seed_value, callback):
        """
        Run a callback with temporary Python/NumPy/Torch RNG seeding.

        This model path rejects an explicit Hugging Face `generator` kwarg, so
        deterministic generation needs to use scoped global RNG seeding.
        """
        if seed_value is None:
            return callback()

        normalized_seed = int(seed_value) % (2 ** 32)
    ...
    def infer(self, spk_audio_prompt, text, output_path,
        ...
        seed_value = generation_kwargs.pop("seed", None)
        seeded_random = random

        if seed_value is not None:
            seed_value = int(seed_value) % (2 ** 32)
            seeded_random = random.Random(seed_value)
            print(f"[DEBUG] Using deterministic inference seed: {seed_value}")

        # Handle emotion text processing separately from emotion vector processing
        if use_emo_text:
            # automatically generate emotion vectors from text prompt
            if emo_text is None:
                emo_text = text  # use main text prompt
            print(f"[DEBUG] Emotion text processing enabled")
            print(f"[DEBUG] Using emotion text: {emo_text}")
            print(f"[DEBUG] QwenEmotion model loaded: {self.qwen_emo is not None}")
            emo_dict = self._get_qwen_emo().inference(emo_text)
            print(f"[DEBUG] detected emotion vectors from text: {emo_dict}")
            # convert ordered dict to list of vectors; the order is VERY important!
            emo_vector = list(emo_dict.values())
            print(f"[DEBUG] Final emotion vector: {emo_vector}")
            # When using emotion text, we must remove external emotion reference audio
            emo_audio_prompt = None
        elif emo_vector is not None:
            # we're using emotion vector guidance; so we must remove
            # "emotion reference voice", to ensure we use correct emotion mixing!
            emo_audio_prompt = None
            print(f"[DEBUG] Using provided emotion vectors: {emo_vector}")
        else:
            print(f"[DEBUG] Emotion text processing DISABLED (use_emo_text={use_emo_text})")
        ...
        if emo_vector is not None:
            ...
            if use_random:
                random_index = [seeded_random.randint(0, x - 1) for x in self.emo_num]
            else:
        ...
        for seg_idx, sent in enumerate(segments):
            segment_base = 0.2 + 0.7 * seg_idx / segments_count
            segment_span = 0.7 / max(1, segments_count)
            self._set_progress(
                segment_base,
            ...
            with torch.no_grad():
                with torch.amp.autocast(text_tokens.device.type, enabled=self.dtype is not None, dtype=self.dtype):
                    self._set_progress(
                        segment_base + segment_span * 0.1,
                        f"conditioning speech {seg_idx + 1}/{segments_count}..."
                    )
                    ...
                    codes, speech_conditioning_latent = self._run_with_deterministic_seed(
                        seed_value,
                        run_inference_speech
                    )
                    self._set_progress(
                        segment_base + segment_span * 0.35,
                        f"planning acoustics {seg_idx + 1}/{segments_count}..."
                    )
                ...
                with torch.amp.autocast(text_tokens.device.type, enabled=self.dtype is not None, dtype=self.dtype):
                    self._set_progress(
                        segment_base + segment_span * 0.55,
                        f"building mel {seg_idx + 1}/{segments_count}..."
                    )
                    latent = self.gpt(
                ...
                with torch.amp.autocast(text_tokens.device.type, enabled=dtype is not None, dtype=dtype):
                    ...
                    self._set_progress(
                        segment_base + segment_span * 0.72,
                        f"refining spectrogram {seg_idx + 1}/{segments_count}..."
                    )
                    ...
                    m_start_time = time.perf_counter()
                    self._set_progress(
                        segment_base + segment_span * 0.88,
                        f"vocoding segment {seg_idx + 1}/{segments_count}..."
                    )
                    ...
                    wav = wav.squeeze(1)
                    self._set_progress(
                        segment_base + segment_span * 0.96,
                        f"finishing segment {seg_idx + 1}/{segments_count}..."
                    )
